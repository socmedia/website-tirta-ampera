...
<div class="fixed inset-0 z-50 m-0 bg-black/20 backdrop-blur-[1px] transition-opacity" x-cloak
     x-show="{{ $modal }}" x-on:click="{{ $modal }} = false; {{ $dismissAction }}"
     x-on:keydown.escape.window="{{ $modal }} = false; {{ $dismissAction }}" wire:key="{{ randAlpha() }}">
</div>
...
<div class="modal {{ $variant }}" role="dialog" tabindex="-1" x-show="{{ $modal }}"
    ...
    <div class="modal-wrapper w-full">
        <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-neutral-800">
            <h3 class="font-bold text-gray-800 dark:text-neutral-200" id="hs-bg-gray-on-hover-cards-label">
                {{ $title }}
            </h3>
