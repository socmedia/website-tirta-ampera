<div class="block antialiased" x-cloak x-data="dropzone({
    ...
    <div
        ...
        <div class="w-full overflow-hidden rounded-xl border border-dashed border-neutral-200">
            <div @dragenter.prevent.document="onDragenter($event)" x-on:click="$refs.input.click()"
                ...
                <div class="flex cursor-pointer flex-col items-center gap-1 py-12 text-center text-neutral-600 dark:bg-neutral-700"
                    ...
                    <input class="hidden" type="file" x-ref="input" wire:model="upload"
                           ...
                           x-on:livewire-upload-error="isLoading = false; isDropped = false"
    ...
    @script
        <script>
                    ...
                    onDrop(e) {
                        ...
                        const args = [
                            ...
                            (error) => {
                                ...
                                if (error && error.detail && error.detail.errors && error.detail.errors
                                    .upload) {
                                    // Laravel validation error (422)
                                    _this.set('error', error.detail.errors.upload);
                                } else if (error && error.message) {
                                    _this.set('error', [error.message]);
                                } else {
                                    _this.set('error', [
                                        'An unknown error occurred during file upload. Please try again or check your file type and size.'
                                    ]);
                                }
                            },
                            () => {
            ...
            // Listen for uploadError event from backend (for 422 validation errors)
            document.addEventListener('livewire:init', () => {
                window.Livewire?.find?.(@this.__instance.id)?.on?.('{{ $uuid }}:uploadError', (message) => {
                    // Set error and stop loading
                    @this.set('error', [message]);
                    const dropzone = document.querySelector('[x-data^="dropzone"]')?.__x;
                    if (dropzone && dropzone.$data) {
                        dropzone.$data.isLoading = false;
                    }
                });
            });
        </script>
