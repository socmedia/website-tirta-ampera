...
@section('content')
    ...
    <div class="container">
        <div class="grid gap-4 md:grid-cols-2 md:items-center md:gap-8 xl:gap-20">
            ...
            <div class="relative ms-4">
                <img class="aspect-[8/9] w-full rounded-b-2xl object-cover" src="{{ getContent('homepage.hero.image') }}"
    ...
    <div class="container py-16 lg:py-36">
        <div class="grid items-center gap-12 md:grid-cols-2">
            <div class="relative">
                <img class="aspect-square rounded-xl object-cover" src="{{ getContent('homepage.about.image') }}"
                     alt="{{ getContent('homepage.about.title') }}">
            ...
            <div>
                <h3 class="text-sm font-semibold uppercase text-sky-600">
                    {{ getContent('homepage.about.subtitle') }}
                </h3>
                <h2 class="mt-3 text-3xl font-bold text-gray-800 lg:text-4xl">
                    {{ getContent('homepage.about.title') }}
                </h2>
                ...
                <a class="mt-8 inline-flex items-center justify-center gap-x-2 rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 focus:bg-gray-50"
                   href="{{ route('front.about') }}" wire:navigate>
        ...
        <div class="container">
            <div class="mx-auto max-w-2xl text-center">
                <h2 class="text-3xl font-bold text-gray-800 lg:text-4xl">
                    {{ getContent('homepage.services.heading') }}
                </h2>
                <p class="mt-3 text-lg text-gray-600">
                    {{ getContent('homepage.services.description') }}
                </p>
            </div>
            @php
                $services = getContent('homepage.services.list') ?? [];
            @endphp
            <div class="mx-auto mt-12 flex flex-wrap justify-center gap-6">
                @foreach ($services as $service)
                    @php
                        ...
                        $linkRoute = $service['link_route'] ?? null;
                        $linkUrl = $linkRoute ? route($linkRoute) : $service['url'] ?? null;
                    @endphp
                    <div class="flex w-full flex-col gap-4 rounded-xl bg-white p-4 lg:w-3/12 lg:p-6">
                        ...
                        <div class="flex flex-1 flex-col">
                            ...
                            @if (!empty($linkLabel) && !empty($linkUrl))
                                <a class="btn soft-primary mt-3 inline-flex items-center gap-x-2"
                                   href="{{ $linkUrl }}" wire:navigate>
                                    {{ $linkLabel }}
