...
@section('title', getContent('seo.move_meter.title'))

@push('meta')
    <x-front::partials.meta :title="getContent('move_meter.header.title')" :description="getContent('move_meter.header.description')" :keywords="getContent('move_meter.header.keywords')" :image="getContent('move_meter.header.image')" />
@endpush
@section('content')
    <x-front::ui.breadcrumb title="Pindah Meter" :items="[
            ...
            <div class="flex w-full flex-col items-center">
                <div class="flex flex-col items-center space-y-4 text-center sm:space-y-6 md:max-w-3xl md:text-center">
                    <p class="badge soft-primary badge-lg rounded-full uppercase">
                        {!! getContent('move_meter.header.badge') !!}
                    </p>
                    <h2 class="text-3xl font-extrabold text-neutral-900 md:text-5xl">
                        {!! getContent('move_meter.header.title') !!}
                    </h2>
                    <p class="text-neutral-500 md:max-w-2xl">
                        {!! getContent('move_meter.header.description') !!}
                    </p>
                </div>
            </div>

            <div class="mx-auto mt-20 grid max-w-5xl gap-8 md:grid-cols-2">
                @foreach (getContent('move_meter.cards', 'value') as $card)
                    <div
