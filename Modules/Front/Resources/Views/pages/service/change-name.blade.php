...
@section('title', getContent('seo.change_name.title'))

@push('meta')
    <x-front::partials.meta :title="getContent('seo.change_name.title')" :description="getContent('seo.change_name.description')" :keywords="getContent('seo.change_name.keywords')" :image="getContent('seo.change_name.image')" />
@endpush

@section('content')
    <x-front::ui.breadcrumb title="Balik Nama Pelanggan" :items="[
        ['label' => __('front::navbar.home'), 'url' => route('front.index')],
        ['label' => __('front::navbar.service'), 'url' => route('front.service.index')],
        ['label' => 'Balik Nama Pelanggan'],
    ]" />
            ...
            <div class="flex w-full flex-col items-center">
                <div class="flex flex-col items-center space-y-4 text-center sm:space-y-6 md:max-w-3xl md:text-center">
                    <p class="badge soft-primary badge-lg rounded-full uppercase">
                        {{ getContent('change_name.header.badge') }}
                    </p>
                    <h2 class="text-3xl font-extrabold text-neutral-900 md:text-5xl">
                        {!! getContent('change_name.header.title') !!}
                    </h2>
                    <p class="text-neutral-500 md:max-w-2xl">
                        {!! getContent('change_name.header.description') !!}
                    </p>
                </div>
            </div>

            <!-- CARDS SECTION (empat kartu, referensi layout replace-stop-valve) -->
            <div class="mx-auto mt-20 grid max-w-5xl gap-8 md:grid-cols-2">
                @foreach (getContent('change_name.cards', 'value', []) as $card)
                    <div
