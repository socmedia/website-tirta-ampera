<div class="space-y-4">
    ...
    <div class="mb-6">
        <h1 class="block text-3xl font-extrabold text-primary-600 dark:text-primary-400">
            👋 {{ __('panel::auth.customer_login_heading') }}
        </h1>
        <p class="mt-2 text-base leading-6 text-gray-600 dark:text-zinc-400">
            {{ __('panel::auth.login_subtitle') }}
        </p>
    </div>

    <form class="grid gap-y-4" wire:submit.prevent="login">
        <x-panel::ui.forms.input type="email" label="{{ __('panel::auth.login_email_label') }}" wire:model.defer="email"
        ...
        <div class="space-y-2 input-group" x-data="{ showPassword: false }">
            <div class="flex justify-between items-center">
                <label class="form-label" for="password">{{ __('panel::auth.login_password_label') }}</label>
                @if (Route::has('auth.customer.password.request'))
                    <x-panel::ui.buttons.link class="text-sm" href="{{ route('auth.customer.password.request') }}"
                                              wire:navigate>
                        {{ __('panel::auth.login_forgot_password') }}
            ...
            <div class="relative">
                <input class="input-field has-icon-right peer" id="password" :type="showPassword ? 'text' : 'password'"
                       wire:model.defer="password" placeholder="{{ __('panel::auth.login_password_label') }}"
                       required />
        ...
        <x-panel::ui.forms.checkbox wire:model="remember" label="{{ __('panel::auth.login_remember_me') }}" />
        ...
        @if (Route::has('auth.customer.register'))
            <p class="mt-2 text-sm text-center text-gray-600 dark:text-zinc-400">
                {{ __('panel::auth.login_no_account') }}
                <x-panel::ui.buttons.link href="{{ route('auth.customer.register') }}" wire:navigate>
                    {{ __('panel::auth.login_register_link') }}
                </x-panel::ui.buttons.link>
            </p>
        @endif
    </form>
    ...
    <div class="text-center">
        <span class="inline-flex gap-1 items-center text-sm text-primary-500 dark:text-primary-300">
            ...
            {{ __('panel::auth.need_help', ['default' => 'Need help?']) }}
            <x-panel::ui.buttons.link href="mailto:{{ getSetting('contact_support_email') }}">
                {{ __('panel::auth.contact_support', ['default' => 'Contact Support']) }}
            </x-panel::ui.buttons.link>
        </span>
    </div>
</div>
